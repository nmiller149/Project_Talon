/*******************************************************************************
* File Name: I2C_Orientation_BOOT.c
* Version 3.50
*
* Description:
...
static uint8 XDATA I2C_Orientation_slReadBuf[I2C_Orientation_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_Orientation_slWriteBuf[I2C_Orientation_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2C_Orientation_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_Orientation_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_Orientation_slReadBuf -  The global variable used to store a response.
...
void I2C_Orientation_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_Orientation_SlaveInitWriteBuf(I2C_Orientation_slWriteBuf, I2C_Orientation_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_Orientation_SlaveInitReadBuf (I2C_Orientation_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2C_Orientation_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_Orientation_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
void I2C_Orientation_CyBtldrCommReset(void) CYSMALL 
{
    ...
    I2C_Orientation_slRdBufSize = 0u;
...
/*******************************************************************************
* Function Name: I2C_Orientation_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2C_Orientation_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;  /* Buffer was copied to I2C buffer */
        ...
        while(0u != timeoutMs)  /* Wait till response is read */
        {
            ...
            CyDelay(I2C_Orientation_WAIT_1_MS);
            timeoutMs--;
        }
    }
...
/*******************************************************************************
* Function Name: I2C_Orientation_CyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_Orientation_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_Orientation_slStatus & I2C_Orientation_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_Orientation_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_Orientation_slWriteBuf,
                              I2C_Orientation_MIN_UNT16(byteCount, size));
